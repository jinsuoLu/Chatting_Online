...
export function RoomChat({ roomId }: { roomId: string }) {
  ...
  useEffect(() => {
    ...
    const socket = io(`${URL}/chat`, { auth: { sessionToken: session.sessionToken }, transports: ['websocket'], reconnection: true });
    ...
    socket.on('error', (event: any) => { setSending(false); setError(event?.message ?? '发送失败'); });
  ...
  function send() {
    ...
    if (!value || value.length > MAX || sending || status !== 'connected' || !socketRef.current) return;
    setSending(true);
