...
describe('RoomsService quotas and lifecycle', () => {
  ...
  it('hides deleted and expired rooms from list queries', async()=>{
    const s = new RoomsService(prisma, jobs);
    await s.list({id:'a1', role:'ADMIN'});
    expect(prisma.room.findMany).toHaveBeenCalledWith(expect.objectContaining({where:expect.objectContaining({adminId:'a1', deletedAt:null, status:{notIn:['DELETED','EXPIRED']}})}));
  });
