...
export class AuditService {
  ...
  async record(input: { action: string; resourceType: string; resourceId?: string; actorUserId?: string; actorVisitorSessionId?: string; metadata?: Record<string, unknown> }) {
    return this.database.auditLog.create({ data: { action: input.action, resourceType: input.resourceType, resourceId: input.resourceId, actorUserId: input.actorUserId, actorVisitorSessionId: input.actorVisitorSessionId, metadata: this.sanitize(input.metadata) as Prisma.InputJsonValue | undefined } });
  }
