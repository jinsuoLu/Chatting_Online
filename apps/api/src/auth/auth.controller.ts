...
export class AuthController {
  ...
  private secureCookie() { return (process.env.WEB_ORIGIN ?? '').startsWith('https://'); }
  private cookie(res:any,token:string){res.cookie('access_session',token,{httpOnly:true,secure:this.secureCookie(),sameSite:'lax',maxAge:15*60*1000,path:'/'});}
  ...
  @Post('logout') @UseGuards(AuthGuard, CsrfGuard) async logout(@Req() req:any,@Res({passthrough:true}) res:any){await this.auth.logout(req.cookies.access_session);res.clearCookie('access_session',{httpOnly:true,sameSite:'lax',secure:this.secureCookie(),path:'/'});return {success:true};}
